...
public class Token {
    ...
    public Token(final String name) {
        ...
        switch (name) {
            ...
            // 上記以外の場合は，43/44/45のどれか 
            default:
            	if (isUnsignedInt(name)) {
            		this.type = "SCONSTANT";
            		this.id = 44;
            	} else if (isPascalString(name)) {
            		this.type = "SSTRING";
            		this.id = 45;
            	} else {
            		// シングルクオートで囲まれていない文字列は識別子と判定
    ...
    // シングルクオーテーションで囲まれた文字列かどうか判定
    private boolean isPascalString(String s) {
